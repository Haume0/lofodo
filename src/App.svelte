<script>
  ...
  import language from "/src/assets/language.json";
  const alarm = new Audio("/alarm.wav");
  const click_on = new Audio("/click_on.wav");
  const click_off = new Audio("/click_off.wav");
  if (localStorage.getItem("lang") == null) {
    if(navigator.language == "tr"){
      localStorage.setItem("lang", "tr_tr");
    }else {localStorage.setItem("lang", "en_us");}
  }
  ...
  function chlang() {
    if(localStorage.getItem('lang_id') == null){localStorage.setItem('lang_id','0')}
    let i = parseInt(localStorage.getItem('lang_id'))
    i += 1
    localStorage.setItem('lang_id',i.toString())
    if(i >= langs.length){
      i = 0
      localStorage.setItem('lang_id','0')
    }
    _lang = langs[i].id
    localStorage.setItem('lang',langs[i].id)
    location.reload();
  }
  let lang = language[_lang];
  const luck = [
  ...
  function startBtn() {
    if (timer.state == lang.start_button) {
      start();
      click_on.play();
    } else if (timer.state == lang.pause_button) {
      pause();
      click_off.play();
    }
  }
...
<main
  class="absolute w-screen noslc flex h-screen !bg-cover !bg-center overflow-hidden bg-black"
>
  <img
    src={setting.background}
    alt=""
    class="absolute w-screen h-screen object-cover blur-[100px]"
  />
  <img
    src={setting.background}
    alt=""
    class="absolute w-screen h-screen md:object-contain sm:object-none sm:object-center"
  />
  <div
    class:!pr-2={showbg == true}
    class="group hover:cursor-pointer ease-in-out duration-500 absolute sm:left-4 md:left-10 md:top-10 sm:top-4 w-auto h-8 bg-black/40 rounded-full flex flex-row"
  >
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    <svg
      on:click={() => (showbg = !showbg)}
      class="hover:fill-sky-500 fill-white mx-1 my-auto"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      ><path
        d="M5 21q-.825 0-1.413-.587Q3 19.825 3 19V5q0-.825.587-1.413Q4.175 3 5 3h5v2H5v14h14v-5.35l2 2V19q0 .825-.587 1.413Q19.825 21 19 21Zm15.85-8.3l-2.4-2.4q-.525.35-1.125.525q-.6.175-1.275.175q-1.85 0-3.15-1.312q-1.3-1.313-1.3-3.188q0-1.875 1.313-3.188Q14.225 2 16.1 2q1.875 0 3.188 1.312Q20.6 4.625 20.6 6.5q0 .675-.2 1.3t-.5 1.15l2.35 2.35q.275.275.275.7q0 .425-.275.7q-.275.275-.7.275q-.425 0-.7-.275ZM16.1 9q1.05 0 1.775-.725q.725-.725.725-1.775q0-1.05-.725-1.775Q17.15 4 16.1 4q-1.05 0-1.775.725Q13.6 5.45 13.6 6.5q0 1.05.725 1.775Q15.05 9 16.1 9ZM7 17q-.3 0-.45-.275q-.15-.275.05-.525l2-2.675q.15-.2.4-.2t.4.2L11.25 16l2.6-3.475q.15-.2.4-.2t.4.2L17.4 16.2q.2.25.05.525Q17.3 17 17 17Z"
      /></svg
    >
    <input
      bind:value={tempBG}
      on:keypress={bcr}
      class:!w-0={showbg == false}
      type="text"
      placeholder="{lang.bg_ph}"
      name="bg"
      id=""
      class="bg-transparent w-[60vw] max-w-[24rem] !border-b-[2px] h-[85%] !border-white ease-in-out duration-500 appearance-none outline-none"
    />
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    <svg
      on:click={() => luckyBG()}
      class:!hidden={showbg == false}
      class="hover:fill-violet-500 fill-white ml-2 my-auto"
      width="18"
      height="24"
      viewBox="0 0 448 472"
      ><path
        d="M399 15Q386 2 369.5 2T340 15L37 316q-13 13-13 30t13 30l30 32q13 13 29.5 13t29.5-13l303-303q13-13 13-30t-13-30zm-91 152l-30-30l89-92l30 30zM67 3Q45 3 45 24v21H24Q3 45 3 67q0 21 21 21h21v21q0 22 22 22q9 0 15-6t6-16V88h21q10 0 16-6t6-15q0-22-22-22H88V24Q88 3 67 3zm298 256q-21 0-21 21v21h-21q-22 0-22 22q0 9 6 15t16 6h21v21q0 10 6 16t15 6q22 0 22-22v-21h21q21 0 21-21q0-22-21-22h-21v-21q0-21-22-21z"
      /></svg
    >
  </div>
  <div
    class:!translate-y-0={show == true}
    class:yt_full={fullsc == true}
  ...
  >
    ...
    <div
      on:click={() => (show = !show)}
      class="absolute left-1/2 right-1/2 -translate-x-1/2 flex place-items-center hover:cursor-pointer justify-items-center z-10 -top-4 self-center bg-black/40 w-12 h-4 rounded-t-xl"
    >
      <svg
        class:rotate-180={show == true}
        class="mx-auto fill-white"
        width="24"
        height="24"
        viewBox="0 0 24 24"><path d="m7 14l5-5l5 5Z" /></svg
      >
    </div>
    {#if setting.yt != ""}<iframe
        class:yt_full={fullsc == true}
        class="relative w-full sm:h-[30vh] md:h-[38vh] z-10 rounded-xl"
        src={"https://www.youtube.com/embed/" +
          getId(setting.yt) +
          "?autoplay=1&controls=1&fs=0"}
        title="YT"
        frameborder="0"
        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
        allowfullscreen
      />{/if}
    {#if setting.sp != ""}<iframe
        class="z-10 "
        about=""
        src={setting.sp
    ...
    <div class="absolute right-1 top-1 flex flex-row gap-1">
      ...
      {#if true}<div
          on:click={() => lucky()}
          class="relative cursor-pointer z-50 w-6 h-6 justify-center place-items-center flex bg-black/60 rounded-full group"
        >
          <svg
            class="group-hover:fill-violet-500 fill-white"
            width="15"
            height="18"
            viewBox="0 0 448 472"
            ><path
      ...
      <!-- svelte-ignore a11y-click-events-have-key-events -->
      {#if rawSource != "" && setting.yt != ""}<div
          on:click={() => (fullsc = !fullsc)}
          class="relative cursor-pointer z-50 w-6 h-6 justify-center place-items-center flex bg-black/60 rounded-full group"
        >
          <svg
            class:hidden={fullsc == false}
            class="group-hover:fill-cyan-400 fill-white"
            width="32"
            height="32"
            viewBox="0 0 24 24"
            ><path
              d="M8 19v-3H5v-2h5v5Zm6 0v-5h5v2h-3v3Zm-9-9V8h3V5h2v5Zm9 0V5h2v3h3v2Z"
            /></svg
          >
          <svg
            class:hidden={fullsc == true}
            class="group-hover:fill-cyan-400 fill-white"
            width="32"
            height="32"
            viewBox="0 0 24 24"
      ...
      <!-- svelte-ignore a11y-click-events-have-key-events -->
      {#if (rawSource != "" && setting.sp != "") || setting.yt != ""}<div
          on:click={() => scrReset()}
          class="relative cursor-pointer z-50 w-6 h-6 justify-center place-items-center flex bg-black/60 rounded-full group"
        >
          <svg
            class="group-hover:stroke-red-500 stroke-white"
            width="32"
            height="32"
            viewBox="0 0 16 16"
            ><path
    ...
    <div class="absolute w-full h-full left-0 top-0">
      ...
      {#if showscr}<div
          in:fly
          out:fly
          class:!w-screen={fullsc == true}
          class="w-full h-[80px] z-10 absolute p-2 flex flex-col bg-black/40 backdrop-blur-sm rounded-xl"
        >
          <span class="mx-auto font-Figtree font-semibold text-white"
            >{lang.player_header}</span
          >
          <div>
            <input
              on:keypress={scr}
              type="text"
              class="absolute max-w-[96%] m-auto text-white  h-8 bg-transparent pl-1 w-full outline-none border-b-[2px] focus:border-b-[2px] placeholder:text-zinc-200 border-white focus:border-[#4CC2CA] ease-in-out duration-500 appearance-none focus:outline-none"
              bind:value={aSource}
              placeholder="{lang.player_ph}"
            />
          </div>
        </div>{/if}
    </div>
  ...
  <div
    class="absolute sm:hidden md:flex flex-col bg-black/20 p-2 w-auto rounded-xl h-auto right-6 bottom-6 z-50"
  >
    <img src="/spicaLofitexed.svg" alt="" class="" />
    <span class="text-zinc-300 w-[100%] text-right font-Figtree"
      >A project by <a
        href="https://github.com/Haume0"
        target="_blank"
        rel="noreferrer"
        class="text-white hover:text-purple-400 w-auto text-right font-medium font-Figtree high"
        >Haume</a
      >.</span
    >
    <div class="flex flex-row gap-2 self-end">
      <div class=" flex bg-black/80 rounded-md flex-col">
        <button on:click={()=>{chlang()}} class="text-white bg-SPbtn px-1 hover:border-white border-[1px] border-transparent ease-in-out duration-200 rounded-md">
          {lang.lang}
        </button>
      </div>
      <a
        href="https://www.patreon.com/Haume"
        target="_blank"
        rel="noreferrer"
        class="text-white font-medium font-Figtree hover:text-orange-600 w-auto text-right high"
        >{lang.donate}</a
      >
    </div>
  </div>
  <div class="relative m-auto p-2 bg-black/10 rounded-xl z-50">
    <div
      class="h-[20px] hover:cursor-pointer mt-2 flex flex-row noslc gap-[2px] place-items-center place-content-center self-center"
    >
      {#if setting.auto == true}
        <svg on:click={() => chg(-1)} width="24" height="24" viewBox="0 0 24 24"
          ><path fill="currentColor" d="M19 12.998H5v-2h14z" /></svg
        >
        <span
          on:click={() => (setting.pomoCount = 0)}
          class="text-white text-sm ease-in-out duration-200 font-Figtree font-semibold text-center"
          >#{setting.pomoCount}/{setting.longCount}</span
        >
        <svg on:click={() => chg(1)} width="24" height="24" viewBox="0 0 16 16"
          ><path
            fill="currentColor"
    ...
    <div class="flex flex-row gap-4 p-2">
      <button
        on:click={() => Mode(setting.pomodoro, "pomodoro")}
        class:btn_active={modebtn == "pomodoro"}
        class="relative sm:w-[5.4rem] sm:h-8 md:w-32 md:h-10 md:text-base sm:text-xs border-white font-Figtree hover:bg-white checked:bg-black hover:text-black font-bold ease-in-out duration-200 border-[2px] rounded-xl"
        >{lang.pomodoro_button}</button
      >
      <button
        on:click={() => Mode(setting.short, "short")}
        class:btn_active={modebtn == "short"}
        class="relative sm:w-[5.4rem] sm:h-8 md:w-32 md:h-10 md:text-base sm:text-xs border-white font-Figtree hover:bg-white hover:text-black font-bold ease-in-out duration-200 border-[2px] rounded-xl"
        >{lang.short_button}</button
      >
      <button
        on:click={() => Mode(setting.long, "long")}
        class:btn_active={modebtn == "long"}
        class="relative sm:w-[5.4rem] sm:h-8 md:w-32 md:h-10 md:text-base sm:text-xs border-white font-Figtree hover:bg-white hover:text-black font-bold ease-in-out duration-200 border-[2px] rounded-xl"
        >{lang.long_button}</button
      >
    </div>
    <h1
      class="text-white ease-in-out duration-200 font-Figtree font-semibold sm:text-[96px] md:text-[128px] text-center w-full"
    >
      {timer.min}:{timer.sec}
    </h1>
    <div class="flex flex-row gap-8 justify-end place-items-center">
      <button
        on:click={() => autoSwitch()}
        class="relative md:w-28 md:h-12 sm:w-24 border-white border-[1px] border-spacing-[1px] sm:h-10 bg-white c-c place-items-center  rounded-full"
        ><div
          class="md:w-12 md:h-12 sm:w-10 sm:h-10 flex ease-in-out duration-500 rounded-full"
          class:slid_OFF={setting.auto == false}
          class:slid_ON={setting.auto == true}
        >
          {#if setting.auto == true}<img
              src="/on.svg"
              class="m-auto"
              alt=""
            />{/if}
          {#if setting.auto == false}<img
              src="/off.svg"
              class="m-auto"
              alt=""
            />{/if}
      ...
      >
      <button on:click={() => reset()} class="group"
        ><svg
          class="fill-white group-hover:cursor-pointer ease-in-out duration-[0ms] group-hover:rotate-[360deg] sm:w-10 sm:h-10 md:w-12 md:h-12 hover:ease-in-out hover:duration-700"
          width="48"
          height="48"
          fill="none"
          viewBox="0 0 48 48"
      ...
      <button
        on:click={() => startBtn()}
        class:btn_deactive={timer.state == lang.pause_button}
        class="bg-white text-black md:w-40 md:h-16 sm:w-28 sm:h-12 sm:rounded-2xl sm:text-2xl md:rounded-3xl md:text-3xl font-Figtree font-extrabold border-[2px] border-transparent hover:ext-white hover:g-black/10 ease-in-out duration-300 hover:order-white"
        >{timer.state}</button
      >
    </div>
  </div>
...
<style>
  ...
  .slid_ON {
    @apply bg-[#4CCA61] translate-x-[calc(100%+28px/2)];
  }
  .slid_OFF {
