...
<main class="absolute w-screen noslc flex h-screen !bg-cover !bg-center overflow-hidden bg-black">
  ...
  <img src="{setting.background}" alt="" class="absolute w-screen h-screen md:object-contain sm:object-none sm:object-center"/>
  <div class:!pr-2={showbg == true} class="group hover:cursor-pointer ease-in-out duration-500 absolute sm:left-4 md:left-10 md:top-10 sm:top-4 w-auto h-8 bg-black/40 rounded-full flex flex-row">
    ...
    <input bind:value={tempBG} on:keypress={bcr} class:!w-0={showbg == false} type="text" placeholder="Enter a background link. *.png, .gif etc." name="bg" id="" class="bg-transparent w-[60vw] max-w-[24rem] !border-b-[2px] h-[85%] !border-white ease-in-out duration-500 appearance-none outline-none">
    <!-- svelte-ignore a11y-click-events-have-key-events -->
  ...
  <div class:!translate-y-0={show == true} class:yt_full={fullsc == true} class="absolute noslc z-50 ease-in-out duration-500 md:bottom-10 sm:bottom-4 md:translate-y-[calc(100%+40px)] sm:translate-y-[calc(150%-24px)] w-[90vw] max-w-[440px] min-h-[80px] md:left-10 sm:left-4 flex flex-col gap-1">
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    ...
    <div class="absolute w-full h-full left-0 top-0">
      ...
      {#if showscr}<div in:fly out:fly class:!w-screen={fullsc == true} class="w-full h-[80px] z-10 absolute p-2 flex flex-col bg-black/40 backdrop-blur-sm rounded-xl">
  ...
  <div class="relative m-auto p-2 bg-black/10 rounded-xl">
    ...
    <div class="flex flex-row gap-4 p-2">
      <button on:click={()=>Mode(setting.pomodoro,'pomodoro')} class:btn_active={modebtn == 'pomodoro'} class="relative sm:w-[5.4rem] sm:h-8 md:w-32 md:h-10 md:text-base sm:text-xs border-white font-Figtree hover:bg-white checked:bg-black hover:text-black font-bold ease-in-out duration-200 border-[2px] rounded-xl">Pomodoro</button>
      <button on:click={()=>Mode(setting.short,'short')} class:btn_active={modebtn == 'short'} class="relative sm:w-[5.4rem] sm:h-8 md:w-32 md:h-10 md:text-base sm:text-xs border-white font-Figtree hover:bg-white hover:text-black font-bold ease-in-out duration-200 border-[2px] rounded-xl">Short Break</button>
      <button on:click={()=>Mode(setting.long,'long')} class:btn_active={modebtn == 'long'} class="relative sm:w-[5.4rem] sm:h-8 md:w-32 md:h-10 md:text-base sm:text-xs border-white font-Figtree hover:bg-white hover:text-black font-bold ease-in-out duration-200 border-[2px] rounded-xl">Long Break</button>
    </div>
    <h1 class="text-white ease-in-out duration-200 font-Figtree font-semibold sm:text-[96px] md:text-[128px] text-center w-full">{timer.min}:{timer.sec}</h1>
    <div class="flex flex-row gap-8 justify-end place-items-center">
      <button on:click={()=>autoSwitch()} class="relative md:w-28 md:h-12 sm:w-24 border-white border-[1px] border-spacing-[1px] sm:h-10 bg-white c-c place-items-center  rounded-full"><div class="md:w-12 md:h-12 sm:w-10 sm:h-10 flex ease-in-out duration-500 rounded-full"class:slid_OFF={setting.auto == false} class:slid_ON={setting.auto == true}>
        {#if setting.auto == true}<img src="/on.svg" class="m-auto" alt="">{/if}
        {#if setting.auto == false}<img src="/off.svg" class="m-auto" alt="">{/if}
      </div></button>
      <button on:click={()=>reset()} class="group"><svg class="fill-white group-hover:cursor-pointer ease-in-out duration-[0ms] group-hover:rotate-[360deg] sm:w-10 sm:h-10 md:w-12 md:h-12 hover:ease-in-out hover:duration-700" width="48" height="48" fill="none" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg"><path d="M40.9428 7.05971C38.4257 4.52311 35.3703 2.58361 32.0021 1.38443C28.6339 0.185245 25.039 -0.242987 21.4829 0.131357C10.4617 1.24109 1.39241 10.179 0.161156 21.1864C-1.49053 35.7329 9.77097 48 23.9755 48C28.5104 48 32.9523 46.7149 36.785 44.2939C40.6178 41.8729 43.684 38.4155 45.6276 34.3232C46.5886 32.3137 45.1471 30.0043 42.9248 30.0043C41.8137 30.0043 40.7626 30.6041 40.2821 31.5939C38.5386 35.3401 35.5564 38.3722 31.8373 40.1803C28.1181 41.9884 23.889 42.462 19.8613 41.5215C13.1945 40.0519 7.81898 34.6232 6.40753 27.9648C5.81422 25.3322 5.82109 22.6 6.42764 19.9704C7.03418 17.3409 8.22488 14.8811 9.91166 12.7731C11.5984 10.6651 13.7382 8.96286 16.1725 7.79219C18.6069 6.62153 21.2736 6.01244 23.9755 6.00996C28.9606 6.00996 33.4051 8.07947 36.6484 11.3487L32.1138 15.8776C30.2218 17.7672 31.5432 21.0064 34.2159 21.0064H44.9969C46.6486 21.0064 48 19.6567 48 18.0071V7.23967C48 4.5703 44.7567 3.22062 42.8648 5.11018L40.9428 7.05971V7.05971Z"/></svg></button>
      <button on:click={()=>startBtn()} class:btn_deactive={timer.state == 'Pause'} class="bg-white text-black md:w-40 md:h-16 sm:w-28 sm:h-12 sm:rounded-2xl sm:text-2xl md:rounded-3xl md:text-3xl font-Figtree font-extrabold border-[2px] border-transparent hover:ext-white hover:g-black/10 ease-in-out duration-300 hover:order-white">{timer.state}</button>
    </div>
  </div>
</main>
